    ...
    <div id="tab_aut">
            ...
            <table class="table table-striped" >
                ...
                <tbody>
                    <tr v-for="(s, indice) in sitacoes" :key ="s.id" :class="{ active: indice == currentIndex }">
                            <td>{{s.id}}</td>
                            <td>{{s.descricao}}</td>
                            <td><button v-on:click="setCurrentSitacao(s, indice)" class="btn" type="button">Alterar</button></td>
                            <td><button v-on:click="remSitacao(s, indice)" class="btn" type="button">Remover</button></td>
                    </tr>
                </tbody>
            </table>
        ...
        <div class="col-md-6">
            <div v-if="currentSitacao">
                    ...
                    :href="'/sitacao/' + currentSitacao.id"
                    >
                    Edit
                    </a>
            </div>
